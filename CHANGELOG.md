...
- **修复从 npm 安装后样式不生效的问题**
  - 移除 `vite-plugin-lib-inject-css` 的 CSS 自动注入机制
  - Taro Vite 构建器不会处理 `node_modules` 中 JS 文件的 CSS import 语句，导致通过 npm 安装时样式无法被正确打包

### ⚠️ 破坏性变更

- **样式需要手动导入**：组件不再自动注入样式，用户需要在项目中显式导入样式文件

### 📖 使用方式

推荐在 `app.scss` 中全局导入样式（SCSS 方式最可靠）：
...
```tsx
// app.ts
import 'taro-virtual-swiper/style.css';
```
